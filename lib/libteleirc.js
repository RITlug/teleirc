...
// Workaround since JavaScript is a terrible language.
// Who would have guessed that the this pointer in something like
// "this.ircbot.say()" in a lambda isn't actually the teleirc instance,
// No, its the instance of whatever is calling the function.
//
// C# master race!
let teleirc = null;
...
class TeleIrc {
    ...
    initStage5_initTelegramMessageSending(){
        ...
        this.tgbot.on('message', this.handleTelegramMsg);
    }
    ...
    handleTelegramMsg(msg) {
        ...
        if (msg.chat.id == teleirc.config.tg.chatId) {
            ...
            if (msg.text === undefined) {
                if (msg.new_chat_member && teleirc.config.irc.showJoinMessage) {
                    ...
                    teleirc.ircbot.say(teleirc.config.irc.channel, teleirc.getTelegramToIrcJoinLeaveMsg(first_name, username, "has joined the Telegram Group!"));
                    ...
                    teleirc.ircbot.say(teleirc.config.irc.channel, teleirc.getTelegramToIrcJoinLeaveMsg(first_name, username, "has left the Telegram Group."));
                    ...
                    if (emoji !== undefined && teleirc.config.irc.sendStickerEmoji) {
                        teleirc.ircbot.say(teleirc.config.irc.channel, teleirc.config.irc.prefix + from + teleirc.config.irc.suffix + " " + emoji);
                    }
                } else {
                ...
                teleirc.ircbot.say(teleirc.config.irc.channel, teleirc.config.irc.prefix + from + teleirc.config.irc.suffix + " " + message);
            }
        } else {
