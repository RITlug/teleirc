...
class TeleIrc {
  ...
  initStage5_initTelegramMessageSending() {
    ...
    this.tgEventListener = new TgEventListener(this.config.chatId);
    ...
    this.tgbot.on('message', this.TgEventListener.ParseMessage.bind(this.TgEventListener));
    this.tgbot.on('edited_message', this.TgEventListener.ParseMessage.bind(this.TgEventListener));
    ...
    this.tgPhotoHandler = new TgPhotoHandler(
      this.config.imgur.useImgurForImageLinks,
      true,
      this.tgbot,
      this.sendMessageToIrc.bind(this)
    );
