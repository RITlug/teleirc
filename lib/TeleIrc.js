const ActionHandler = require('./Irchandlers/ActionHandler');
const ErrorHandler = require('./Irchandlers/ErrorHandler');
const JoinHandler = require('./IrcHandlers/JoinHandler');
const KickHandler = require('./Irchandlers/KickHandler');
const MessageHandler = require('./IrcHandlers/MessageHandler');
const PartHandler = require('./IrcHandlers/PartHandler');
...
class TeleIrc {
  ...
  initStage4_addIrcListeners() {
    this.messageHandler = new MessageHandler(
      ...
      true,
      this.tgRateLimiter.queueMessage
    );
    this.ircbot.addListener('message', this.messageHandler.ReportMessage);

    this.nickServHandler = new IrcNickServHandler(this.config.irc, this.ircbot);
    this.ircbot.addListener('message', this.nickServHandler.HandleNickServ);

    this.errorHandler = new ErrorHandler(
      true,
      (message) => console.log(message)
    );
    this.ircbot.addListener('error', this.errorHandler.ReportError);
    ...
    this.ircbot.addListener('action', this.actionHandler.ReportAction);

    this.joinHandler = new JoinHandler(
      this.config.tg.showJoinMessage,
      this.tgRateLimiter.queueMessage
    );
    this.ircbot.addListener('join', this.joinHandler.ReportJoin);

    this.partHandler = new PartHandler(
      this.config.tg.showLeaveMessage,
      this.tgRateLimiter.queueMessage
    );
    this.ircbot.addListener('part', this.partHandler.ReportPart);

    this.kickHandler = new KickHandler(
      this.config.tg.showKickMessage,
      this.tgRateLimiter.queueMessage
    );
    this.ircbot.addListener('kick', this.kickHandler.ReportKick);
  }
