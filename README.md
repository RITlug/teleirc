...
| [Offical Node 6 Slim](https://hub.docker.com/r/_/node/) (`node:6-slim`) -- See `Dockerfile`                  | 256 MB   |
| [Official Fedora Latest](https://hub.docker.com/r/_/fedora/) -- See `Dockerfile.fedora`                      | 1.1 GB   |

The below example uses alpine, replace `alpine` with `fedora` or Simply remove use `Dockerfile` for the `node:6-slim` if you so choose.
...
You can optionally, tell Docker Compose to use the Fedora or the official `node:6-slim` image as well by simply replacing the `Dockerfile.alpine` with `Dockerfile.fedora` or `Dockerfile`. It is worth noting that the Fedora image is rather large at over **1 GB** and thus we recommend you use either `Dockerfile.alpine` or `Dockerfile`.
